...
(() => {
  ...
  function installCoverRotationController() {
    const KEY = "glowify-cover-rotation";
    const SELECTORS = [
    ...
    const ANIM = "glowifySpin";
    const MARK = "data-glowify-cover-rot";
    const SAVED = "data-glowify-cover-rot-orig";
    let enabled = (localStorage.getItem(KEY) || "off") === "on" ? "on" : "off";
    let running = true;
    let epoch = 0;
    let frozen = 0;
    let observer = null;
    ...
    const ensureKeyframes = () => {
      ...
      style.id = STYLE_ID;
      style.textContent = `@keyframes ${ANIM}{from{transform:rotate(0)}to{transform:rotate(360deg)}}`;
      document.head.appendChild(style);
    };
    const listTargets = () => {
      const out = [];
      for (const s2 of SELECTORS) document.querySelectorAll(s2).forEach((n) => out.push(n));
      return out;
    };
    const phaseMs = () => {
      if (!running) return frozen;
      const raw = (Date.now() - epoch) % SPEED;
      return raw < 0 ? raw + SPEED : raw;
    };
    const saveOriginal = (el) => {
      if (el.getAttribute(SAVED)) return;
      const props = ["border-radius", "transform-origin", "will-change", "animation", "animation-delay", "animation-play-state"];
      const saved = {};
      for (const p2 of props) saved[p2] = { v: el.style.getPropertyValue(p2), pr: el.style.getPropertyPriority(p2) };
      el.setAttribute(SAVED, JSON.stringify(saved));
    };
    const restoreOriginal = (el) => {
      var _a, _b, _c, _d;
      const raw = el.getAttribute(SAVED);
      const props = ["border-radius", "transform-origin", "will-change", "animation", "animation-delay", "animation-play-state"];
      if (!raw) return;
      try {
        const saved = JSON.parse(raw);
        for (const p2 of props) {
          const v2 = (_b = (_a = saved == null ? void 0 : saved[p2]) == null ? void 0 : _a.v) != null ? _b : "";
          const pr = (_d = (_c = saved == null ? void 0 : saved[p2]) == null ? void 0 : _c.pr) != null ? _d : "";
    ...
    const applySpin = (el, delay) => {
      ...
      el.style.setProperty("animation", `${ANIM} ${SPEED / 1e3}s linear infinite`);
    ...
    const applyAll = () => {
      ...
      ensureKeyframes();
      const delay = -phaseMs() / 1e3;
      for (const el of listTargets()) applySpin(el, delay);
    };
    const scheduleApply = () => {
      if (enabled !== "on" || scheduled) return;
      scheduled = true;
    ...
    const setPlayState = (nextRunning) => {
      ...
      if (!nextRunning) {
        ...
        running = true;
        epoch = Date.now() - frozen;
      }
      document.querySelectorAll(`[${MARK}="1"]`).forEach((n) => n.style.setProperty("animation-play-state", running ? "running" : "paused"));
    };
    const hookPlayPause = () => {
      const anyWin = window;
      ...
      const tryHook = () => {
        ...
        setPlayState(!!((_a = player.isPlaying) == null ? void 0 : _a.call(player)));
        player.addEventListener("onplaypause", () => {
          var _a2;
          return setPlayState(!!((_a2 = player.isPlaying) == null ? void 0 : _a2.call(player)));
        });
        return true;
      };
      if (tryHook()) return;
    ...
    const stop = () => {
      ...
      observer = null;
      document.querySelectorAll(`[${MARK}="1"]`).forEach((n) => restoreOriginal(n));
      for (const el of listTargets()) restoreOriginal(el);
    };
    const setEnabled = (next) => {
      enabled = next === "on" ? "on" : "off";
    ...
    window.addEventListener("glowifyCoverRotationChange", () => {
      const next = (localStorage.getItem(KEY) || "off") === "on" ? "on" : "off";
      setEnabled(next);
    });
