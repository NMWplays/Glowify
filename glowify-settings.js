...
(() => {
  ...
  var glowifySelectScrollLockCount = 0;
  var glowifySelectScrollLockPrevOverflowY = null;
  var glowifySelectScrollLockTarget = null;
  function lockGlowifySettingsScroll(anchor) {
    var _a, _b, _c, _d;
    const panel = (_b = (_a = anchor == null ? void 0 : anchor.closest) == null ? void 0 : _a.call(anchor, ".glowifySettingsPanel")) != null ? _b : null;
    const body = (_d = (_c = panel == null ? void 0 : panel.querySelector) == null ? void 0 : _c.call(panel, ".glowifySettingsBody")) != null ? _d : null;
    if (!body) return () => {
    };
    glowifySelectScrollLockCount += 1;
    if (glowifySelectScrollLockCount === 1) {
      glowifySelectScrollLockTarget = body;
      glowifySelectScrollLockPrevOverflowY = body.style.overflowY;
      body.style.overflowY = "hidden";
    }
    return () => {
      glowifySelectScrollLockCount = Math.max(0, glowifySelectScrollLockCount - 1);
      if (glowifySelectScrollLockCount !== 0) return;
      const target = glowifySelectScrollLockTarget;
      if (target) target.style.overflowY = glowifySelectScrollLockPrevOverflowY || "";
      glowifySelectScrollLockTarget = null;
      glowifySelectScrollLockPrevOverflowY = null;
    };
  }
  function sleep(ms) {
  ...
  function installCoverRotationController() {
    const STORAGE_KEY = "glowify-cover-rotation";
    const SELECTORS = [
      ".main-nowPlayingView-coverArt",
      ".sb59T76Xcd92L_RhBcdz",
      ".nFcVFIiSLs2b22OKTska",
      ".d1ytmgea2yryYrj2wOlw"
    ];
    const ROTATION_SPEED_MS = 5e3;
    const STYLE_ID = "glowify-cover-rotation-keyframes";
    const ANIM_NAME = "glowifySpin";
    const MARK_ATTR = "data-glowify-cover-rot";
    const SAVED_ATTR = "data-glowify-cover-rot-orig";
    let running = true;
    let phaseEpochMs = 0;
    let frozenPhaseMs = 0;
    const getPhaseMs = () => {
      if (!running) return frozenPhaseMs;
      const raw = (Date.now() - phaseEpochMs) % ROTATION_SPEED_MS;
      return raw < 0 ? raw + ROTATION_SPEED_MS : raw;
    };
    const getDelaySeconds = () => -getPhaseMs() / 1e3;
    ...
    const saveOriginal = (el) => {
      if (!el) return;
      if (el.getAttribute(SAVED_ATTR)) return;
      const props = [
        "border-radius",
        "transform-origin",
        "will-change",
      ...
      for (const p2 of props) {
        saved[p2] = {
          value: el.style.getPropertyValue(p2),
          priority: el.style.getPropertyPriority(p2)
        };
      }
      try {
        el.setAttribute(SAVED_ATTR, JSON.stringify(saved));
      } catch (e) {
        el.setAttribute(SAVED_ATTR, "1");
      }
    };
    const restoreOriginal = (el) => {
      ...
      if (!raw || raw === "1") {
        for (const p2 of props) el.style.removeProperty(p2);
      ...
      try {
        ...
        for (const p2 of props) {
          const v2 = (_b = (_a = saved == null ? void 0 : saved[p2]) == null ? void 0 : _a.value) != null ? _b : "";
          const pr = (_d = (_c = saved == null ? void 0 : saved[p2]) == null ? void 0 : _c.priority) != null ? _d : "";
          if (v2) el.style.setProperty(p2, v2, pr);
          else el.style.removeProperty(p2);
        }
      } catch (e) {
        for (const p2 of props) el.style.removeProperty(p2);
      }
    ...
    const applySpin = (el, delaySeconds) => {
      if (!el) return;
      const already = el.getAttribute(MARK_ATTR) === "1";
      if (!already) saveOriginal(el);
      el.style.setProperty("border-radius", "500px", "important");
      el.style.setProperty("transform-origin", "50% 50%");
      el.style.setProperty("will-change", "transform");
      const anim = `${ANIM_NAME} ${ROTATION_SPEED_MS / 1e3}s linear infinite`;
      if (!already || (el.style.getPropertyValue("animation") || "").indexOf(ANIM_NAME) === -1) {
        el.style.setProperty("animation", anim);
        el.style.setProperty("animation-delay", `${delaySeconds}s`);
      }
      el.style.setProperty("animation-play-state", running ? "running" : "paused");
      el.setAttribute(MARK_ATTR, "1");
    };
    let observer = null;
    let enabled = (localStorage.getItem(STORAGE_KEY) || "off") === "on" ? "on" : "off";
    const applyAll = () => {
    ...
    const setPlayState = (nextRunning) => {
      if (running === nextRunning) return;
      if (!nextRunning) {
        frozenPhaseMs = getPhaseMs();
        running = false;
      } else {
        running = true;
        phaseEpochMs = Date.now() - frozenPhaseMs;
      }
      try {
        document.querySelectorAll(`[${MARK_ATTR}="1"]`).forEach((n) => n.style.setProperty("animation-play-state", running ? "running" : "paused"));
      } catch (e) {
      }
    };
    const hookSpicetifyPlayPause = () => {
      const anyWin = window;
      if (anyWin._glowifyCoverRotationHooked) return;
      const tryHook = () => {
        var _a;
        const player = Spicetify == null ? void 0 : Spicetify.Player;
        if (!(player == null ? void 0 : player.addEventListener)) return false;
        anyWin._glowifyCoverRotationHooked = true;
        try {
          const isPlaying = !!((_a = player == null ? void 0 : player.isPlaying) == null ? void 0 : _a.call(player));
          if (!isPlaying) {
            running = true;
            phaseEpochMs = 0;
            frozenPhaseMs = Date.now() % ROTATION_SPEED_MS;
            setPlayState(false);
          } else {
            running = true;
            phaseEpochMs = 0;
      ...
      const timer = setInterval(() => {
        tries++;
        if (tryHook() || tries > 40) clearInterval(timer);
      }, 250);
    };
    const start = () => {
      if (observer) return;
    ...
    const stop = () => {
      ...
      observer = null;
      try {
        document.querySelectorAll(`[${MARK_ATTR}="1"]`).forEach((n) => restoreOriginal(n));
      } catch (e) {
      }
      for (const el of listTargets()) restoreOriginal(el);
    };
    const setEnabled = (next) => {
      const m2 = next === "on" ? "on" : "off";
      enabled = m2;
      localStorage.setItem(STORAGE_KEY, m2);
      if (m2 === "on") start();
      else stop();
    };
    if (enabled === "on") start();
    window.addEventListener("glowifyCoverRotationChange", () => {
      const next = (localStorage.getItem(STORAGE_KEY) || "off") === "on" ? "on" : "off";
      setEnabled(next);
    });
    return { setEnabled, getEnabled: () => enabled, start, stop, applyAll };
  }
  ...
  function updatePlaylistHeaderCss(show) {
    const css = show ? `.main-entityHeader-container.gmKBgPCnX785KDicbdJu { backdrop-filter: blur(1rem) !important; box-shadow: 0 0 25px 8px var(--glowify-glow-accent, var(--accent-color)) !important; }` : `.main-entityHeader-container.gmKBgPCnX785KDicbdJu { backdrop-filter: none !important; box-shadow: none !important; }`;
  ...
  var glowifyTranslations = {
    de: {
      ...
      coverRotation: "Rotierende Cover-Art:",
    ...
    en: {
      ...
      coverRotation: "Rotating cover art:",
      playerRadius: "Player Border Radius:",
    ...
    ru: {
      ...
      coverRotation: "\u0412\u0440\u0430\u0449\u0430\u044E\u0449\u0430\u044F\u0441\u044F \u043E\u0431\u043B\u043E\u0436\u043A\u0430:",
    ...
    es: {
      ...
      coverRotation: "Portada giratoria:",
    ...
    fr: {
      settingsTitle: "Param\xE8tres Glowify",
      title: "Param\xE8tres Glowify",
      ...
      coverRotation: "Pochette rotative:",
      ...
      lyricsMode: "Traduction/romanisation des paroles:",
      lyricsOptions: {
        off: "D\xE9sactiv\xE9",
        translation: "Traduction uniquement",
        romanization: "Romanisation uniquement",
        both: "Traduction + romanisation"
      },
      coverRotationOptions: {
        off: "D\xE9sactiv\xE9",
        on: "Activ\xE9"
    ...
    pt: {
      ...
      coverRotation: "Capa girat\xF3ria:",
    ...
    tr: {
      ...
      coverRotation: "D\xF6nen kapak g\xF6rseli:",
      ...
      lyricsSection: "\u015Eark\u0131 s\xF6zleri",
      lyricsMode: "\u015Eark\u0131 s\xF6z\xFC \xE7eviri/romanizasyon:",
      lyricsOptions: {
        off: "Kapal\u0131",
        translation: "Sadece \xE7eviri",
        romanization: "Sadece romanizasyon",
    ...
    hi: {
      ...
      coverRotation: "\u0918\u0942\u092E\u0924\u093E \u0939\u0941\u0906 \u0915\u0935\u0930 \u0906\u0930\u094D\u091F:",
      ...
      lyricsSection: "Lyrics",
      lyricsMode: "Lyrics \u0905\u0928\u0941\u0935\u093E\u0926/\u0930\u094B\u092E\u0928\u093E\u0907\u091C\u093C\u0947\u0936\u0928:",
      lyricsOptions: {
        off: "\u092C\u0902\u0926",
  ...
  function ensureSettingsUiStyle() {
    ...
    .glowifyRowControls .glowifySelectBtn { flex: 0 0 auto; }
  ...
  function Select(props) {
    ...
    React.useEffect(() => {
      if (!open) return;
      return lockGlowifySettingsScroll(btnRef.current);
    }, [open]);
    const useLayout = React.useLayoutEffect || React.useEffect;
    useLayout(() => {
      if (!open) return;
      ...
      const recalc = () => {
        ...
        const panelBounds = (body == null ? void 0 : body.getBoundingClientRect) ? body.getBoundingClientRect() : panel ? panel.getBoundingClientRect() : { left: 0, top: 0, right: window.innerWidth, bottom: window.innerHeight };
        ...
        const maxPossible = Math.max(0, viewportBounds.right - viewportBounds.left - 16);
        const width = Math.min(wantedWidth, maxPossible);
        const top = r.bottom + margin;
        const inView = r.bottom > panelBounds.top + 4 && r.top < panelBounds.bottom - 4;
        if (!inView) {
          setPos(null);
          setOpen(false);
          return;
        ...
        const minLeft = viewportBounds.left + 8;
        const maxLeft = viewportBounds.right - width - 8;
        left = Math.min(Math.max(left, minLeft), maxLeft);
        const belowSpace = viewportBounds.bottom - top - 8;
        const itemH = 40;
        const wantedHeight = props.options.length * itemH;
        const maxHeight = Math.max(80, Math.min(wantedHeight, Math.min(320, belowSpace)));
        const needsScroll = wantedHeight > maxHeight + 1;
        setPos({ left, top, width, maxHeight, needsScroll });
      };
      recalc();
      window.addEventListener("resize", recalc);
      window.addEventListener("scroll", recalc, true);
    ...
    const menu = open && pos ? /* @__PURE__ */ React.createElement(
      "div",
      {
        ...
        style: {
          ...
          width: `${pos.width}px`,
          maxHeight: `${pos.maxHeight}px`,
          overflowY: pos.needsScroll ? "auto" : "hidden",
          transform: void 0
        }
      },
  ...
  function SettingsContent(props) {
    ...
    const titles = {
      ...
      lyrics: t.lyricsSection || (de ? "Lyrics" : "Lyrics")
    };
    ...
    const [coverRotationMode, setCoverRotationMode] = React.useState(
      readLS("glowify-cover-rotation", "off") || "off"
    );
    ...
    const [lyricsMode, setLyricsMode] = React.useState(
      readLS("glowify-lyrics-mode", "romanization") || "romanization"
    );
    ...
    const applyArtistMode = async (mode) => {
      ...
      (_c = (_b2 = props.artistCtrl) == null ? void 0 : _b2.setMode) == null ? void 0 : _c.call(_b2, mode);
    };
    ...
    const applyCoverRotationMode = (mode) => {
      const m2 = mode === "on" ? "on" : "off";
      setCoverRotationMode(m2);
      localStorage.setItem("glowify-cover-rotation", m2);
      window.dispatchEvent(new Event("glowifyCoverRotationChange"));
    };
    ...
    const applyLyricsMode = (m2) => {
      setLyricsMode(m2);
      localStorage.setItem("glowify-lyrics-mode", m2);
    ...
    return /* @__PURE__ */ React.createElement("div", { className: "glowifySettingsPanel" }, /* @__PURE__ */ React.createElement("div", { className: "glowifySettingsHeader" }, /* @__PURE__ */ React.createElement("h3", { className: "glowifySettingsTitle" }, t.title), /* @__PURE__ */ React.createElement("div", { className: "glowifyHeaderActions" }, /* @__PURE__ */ React.createElement(
    ...
    )))), /* @__PURE__ */ React.createElement(Section, { title: titles.glow }, /* @__PURE__ */ React.createElement("div", { className: "glowifyRow" }, /* @__PURE__ */ React.createElement("div", { className: "glowifyLabel" }, t.glowColor), /* @__PURE__ */ React.createElement("div", { className: "glowifyRowControls" }, /* @__PURE__ */ React.createElement(
      Select,
      {
        value: glowMode,
        options: [
      ...
      "input",
      {
        ...
        onChange: async (e) => {
          ...
          if (!file) return;
          await applyCustomBackground(file);
          (_c = (_b2 = props.artistCtrl) == null ? void 0 : _b2.applySavedModeIfArtist) == null ? void 0 : _c.call(_b2);
        }
      }
    ...
    ))), /* @__PURE__ */ React.createElement("div", { className: "glowifyRow" }, /* @__PURE__ */ React.createElement("div", { className: "glowifyLabel" }, t.playerRadius), /* @__PURE__ */ React.createElement(Stepper, { value: playerRadius, min: 0, max: 100, onChange: applyRadius })), /* @__PURE__ */ React.createElement("div", { className: "glowifyRow" }, /* @__PURE__ */ React.createElement("div", { className: "glowifyLabel" }, t.coverRotation), /* @__PURE__ */ React.createElement(
      Select,
      {
        value: coverRotationMode,
        options: [
          { value: "off", label: t.coverRotationOptions.off },
          { value: "on", label: t.coverRotationOptions.on }
        ],
        onChange: applyCoverRotationMode
      }
    ))), /* @__PURE__ */ React.createElement(Section, { title: titles.playlist }, /* @__PURE__ */ React.createElement("div", { className: "glowifyRow" }, /* @__PURE__ */ React.createElement("div", { className: "glowifyLabel" }, t.playlistHeaderBox), /* @__PURE__ */ React.createElement(
      Select,
      {
    ...
    ))), /* @__PURE__ */ React.createElement(Section, { title: titles.lyrics }, /* @__PURE__ */ React.createElement("div", { className: "glowifyRow" }, /* @__PURE__ */ React.createElement("div", { className: "glowifyLabel" }, t.lyricsMode), /* @__PURE__ */ React.createElement("div", { className: "glowifyRowControls" }, /* @__PURE__ */ React.createElement(
      Select,
      {
        value: lyricsMode,
        options: [
          { value: "off", label: t.lyricsOptions.off },
          { value: "translation", label: t.lyricsOptions.translation },
          { value: "romanization", label: t.lyricsOptions.romanization },
          { value: "both", label: t.lyricsOptions.both }
        ],
        onChange: (v2) => applyLyricsMode(v2)
      }
  ...
  (async function initGlowifyStandaloneTs() {
    ...
    if (!anyWin.glowifyCoverRotationCtrl) {
      try {
        anyWin.glowifyCoverRotationCtrl = installCoverRotationController();
      } catch (e) {
        console.error("Glowify cover rotation init failed", e);
      }
    }
    if (!anyWin.glowifyLyricsTranslatorCtrl) {
      try {
        anyWin.glowifyLyricsTranslatorCtrl = installLyricsTranslator();
      } catch (e) {
        console.error("Glowify lyrics translator init failed", e);
      }
    }
