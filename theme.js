...
(async function () {
  ...
  async function updateBackgroundAndAccent() {
    ...
    const changedCustom = bgMode !== lastBgMode || customImage !== lastCustomImage || bgUrl !== lastBgUrl;
    ...
    if (coverUrl) {
      ...
      const enhanced = enhanceColor(color, satBoost, lightBoost);
      document.documentElement.style.setProperty("--accent-color", enhanced);
      document.documentElement.style.setProperty("--glowify-accent-raw", color);
      if (localStorage.getItem("glowify-glow-mode") !== "custom") {
        const glowSat = (parseFloat(localStorage.getItem("glowify-glow-saturation-boost")) || 17) / 10;
        const glowLight = (parseFloat(localStorage.getItem("glowify-glow-lightness-boost")) || 11) / 10;
        const glowEnhanced = enhanceColor(color, glowSat, glowLight);
        document.documentElement.style.setProperty("--glowify-glow-accent", glowEnhanced);
      }
    }
  }

  async function reapplyAccentOnly() {
    const coverUrl = getCoverUrl();
    if (!coverUrl) return;
    const color = await getDominantColor(coverUrl);
    const satBoost = (parseFloat(localStorage.getItem("glowify-accent-saturation-boost")) || 17) / 10;
  ...
  window.addEventListener("glowifyAccentBoostChange", reapplyAccentOnly);
